...
def get_token_number(tokens, pos_tags):
    """
    Unified function to determine if a token sequence is singular or plural.
    Used for both subjects and noun phrases.
    Returns: 'singular', 'plural', or 'unknown'
    """
    if not tokens:
        return 'unknown'
    
    # Get the head word (last token in the phrase, typically the noun)
...
def get_verb_base_form(verb):
    """
    Get base form of a verb using NLTK's WordNetLemmatizer.
    Falls back to simple heuristics if lemmatizer fails.
    """
    try:
